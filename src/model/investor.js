...
var investor = {
    create: function (params, cb) {
        ...
        investorModel.create(params, function (err, result) {
          if (err) {
            console.log(err);
            return cb(
              ec.appError({
      ...
      fetchAll: function (cb) {
        investorModel.find(function(err, result) {
          if (err) {
              console.log(err);
      ...
      fetchById: function(id, cb) {
        investorModel.find({_id: id}, function(err, result) {
            if (err) {
                console.log(err);
                return cb(ec.appError({
                    status: ec.DB_ERROR,
    ...
    update: function (id, updateData, cb) {
      ...
      investorModel.updateOne({ _id: id }, { $set: updateData }, function (
        err,
        result
      ) {
        if (err) {
    ...
    deleteId: function(params, cb) {
      investorModel.deleteOne({_id: params.id}
        , function(err, result) {
          if (err) {
              return cb(ec.appError({
                  status: ec.DB_ERROR,
